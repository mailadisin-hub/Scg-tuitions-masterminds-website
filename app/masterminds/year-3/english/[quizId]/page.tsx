...
import { year3Comprehensions, getComprehensionById } from "@/lib/comprehensions";
import ComprehensionQuiz from "@/app/components/ComprehensionQuiz";

export async function generateStaticParams() {
  return year3Comprehensions.map((c) => ({ quizId: c.id }));
}

export default async function Year3QuizPage({ params }: { params: Promise<{ quizId: string }> }) {
  const { quizId } = await params;
  const comprehension = getComprehensionById(quizId);

  if (!comprehension || comprehension.year !== 3) {
    notFound();
  }
    ...
    <div>
      <div className="py-4 px-4" style={{ backgroundColor: "#f0faf0", borderBottom: "1px solid #c8e6c9" }}>
        <div className="max-w-3xl mx-auto flex items-center gap-2 text-sm text-gray-500">
          ...
          <span>›</span>
          <Link href="/masterminds/year-3/english" className="hover:text-green-700">
            Year 3 English
          </Link>
          <span>›</span>
